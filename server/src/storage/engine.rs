...
impl StorageEngine {
    ...
    async fn load_inode_attributes(&self, inode: u64) -> Result<InodeAttributes> {
        let inode_path = self
            .data_root
            .join("inodes")
            .join(format!("{}.json", inode));
        let content = fs::read_to_string(&inode_path)
            .await
...
mod tests {
    ...
    async fn test_create_nested_file() {
        ...
        engine
            .create_file("/dir1/dir2/file.txt", b"nested content")
            .await
            .unwrap();
    ...
    async fn test_list_directory() {
        ...
        engine.create_directory("/test_dir").await.unwrap();
        engine
            .create_file("/test_dir/file1.txt", b"content1")
            .await
            .unwrap();
        engine
            .create_file("/test_dir/file2.txt", b"content2")
            .await
            .unwrap();
    ...
    async fn test_delete_non_empty_directory_fails() {
        ...
        engine.create_directory("/test_dir").await.unwrap();
        engine
            .create_file("/test_dir/file.txt", b"content")
            .await
            .unwrap();
